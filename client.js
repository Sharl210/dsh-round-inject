 ...
 *
 * Data channel: `ctx.configForms.get('round-inject')`, the shared per-entry form
 * service dsh-client-ui-settings owns (the old `settingsScope` service was
 * removed in DSH 0.1.7). Its snapshot carries the same status/value/writable
 * shape this page was written against, and its `set` writes are ordered and
 * revision-fenced. No custom RPC.
 */
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      // Registered while the Host actually serves the entry: the form reports
      // `status: 'unavailable'` until the describe mirror carries the
      // namespace, and the page would otherwise show its unavailable notice on
      // a loaded plugin. Both surfaces share the one scope.
      ctx.effect(
        () =>
          ctx.configForms.whileServed([NAMESPACE], () => {
            ...
            // The bundle row's own configuration page on the Plugins page.
            const offRow = ctx.slots.inject('plugins.row.config', () =>
              ctx.slots.register(
                { name: 'plugins.row.config', key: 'dsh-round-inject#round-inject' },
                (props) => h(RoundInjectSettings, { ...props, scope }),
              ),
            )
            return () => {
        ...
        'round-inject: settings page',
      )
    }

    exports.apply = apply
    exports.inject = ['configForms', 'slots']
